...
public class CorpusPropertiesDialog extends TitledPageApplicationDialog {
  ...
  private CorpusPropertiesDialog(CorpusEntity corpus, String dialogId) {
    ...
    setDialogPage(compositePage);//new FormBackedDialogPage(corpusForm));
...
//      formBuilder.add("language");
  ...
  private class AudioFormatFormat extends AbstractForm {
    ...
    protected JComponent createFormControl() {
      ...
      formBuilder.row();
      formBuilder
          .add(bf.createBoundComboBox("audioFormat.encoding", dictSource.getItems("audio.format.encoding"), "text"));
      formBuilder.row();
      formBuilder.add(
          bf.createBoundComboBox("audioFormat.samplingRate", dictSource.getItems("audio.format.samplingRate"), "text"));
      formBuilder.row();

      formBuilder
          .add(bf.createBoundComboBox("audioFormat.sampleSize", dictSource.getItems("audio.format.sampleSizeInBits"),
              "text"));
      formBuilder.row();
      formBuilder.add(
          "audioFormat.signed");//bf.createBoundComboBox("audioFormat.signed", dictSource.getItems("audio.format.signed"), "text"));
      formBuilder.row();
