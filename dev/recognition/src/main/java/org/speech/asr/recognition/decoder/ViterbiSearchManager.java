...
public class ViterbiSearchManager implements SearchManager {
  ...
  protected boolean comparePaths(List<SearchNode> path1, List<SearchNode> path2) {
    ...
    return last1 == last2;
   //todo w celach testowych czasozerna ale poprawna implementacja zostala zakomentowana
/*    Iterator<SearchNode> iter1 = path1.iterator();
    Iterator<SearchNode> iter2 = path2.iterator();

    while (iter1.hasNext()) {
      SearchNode node1 = iter1.next();
      SearchNode node2 = iter2.next();
      if (node1 != node2) {
        return false;
      }
    }
