 ...
 * <p/>
 * Creation date: Aug 29, 2009 <br/>
 * <a href="http://www.grapesoftware.com">www.grapesoftware.com</a>
 *
 * @author Lukasz Olczak
 */
public class An4CvtStuTest {
  /**
   * slf4j Logger.
   */
  private static final Logger log = LoggerFactory.getLogger(An4CvtStuTest.class.getName());
  ...
  public void start() {
    noDimensions = 39;
    noOutputs = 483;//micro=2x224trans i 35 stat; full=927;//35 stationary i 892-transition
    logParameters();
    ...
    createModel();
    transcriptionFactory = new StuTranscriptionFactory(model, dictionary);
    try {
  ...
  private void logParameters() {
    ...
    log.info("No states per stationary: {}", noStatesPerStationary);
    log.info("No states per transition: {}", noStatesPerTransition);
  ...
  protected NeuralNetwork buildNet() {
    ...
    builder.addLayer("output", LayerType.SOFT_MAX, noOutputs, LayerPosition.OUTPUT);
...
//    assert noOutputs == phoneSet.size();
    ...
    for (String symbol : phoneSet) {
      ...
      double logPrior = 0.0;//logScale.linearToLog(priorEstimator.getPrior(symbol));
      log.info("Setting log prior probability for class {} = {}", symbol, logPrior);

      int noStatesPerPhoneme = symbol.contains("-") ? noStatesPerTransition : noStatesPerStationary;
